...
reviews_df = pd.read_csv("reviews_prediction.csv")
reviews_df.head()
...
X = reviews_df.iloc[:, reviews_df.columns != "is_review"]
Y = reviews_df["is_review"]
...
# a decision tree model with default values
dtc = DTC('entropy', max_depth= 7, max_features= None, min_samples_split= 3)
...
train_score = lreg.score(X_train, Y_train)
# print the R2 of training data
print("Logistic regression R2 (after tuning hyperparameters)(Train) = " + str(round(train_score, 4)))
...
# check the accuracy metrics again
# a logistic regression model with specific values
knn = KNN(algorithm = 'auto', leaf_size = 30, n_neighbors = 1)
# fit the model using some training data
knn_fit = knn.fit(X_train, Y_train)
# generate a mean accuracy score for the predicted data
